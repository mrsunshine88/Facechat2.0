...
import { Mail, Book, Users, Star, Settings, User, FileEdit, Search, Shield, HelpCircle, AlertTriangle, CheckCircle, Eye, Camera, Clock, Trash2, Eraser, ShieldAlert, MessageSquare, Moon, Heart, Zap, Coffee, Ghost, Sun, Gamepad2, Undo, Music, Volume2, VolumeX } from 'lucide-react';
...
const KIDS_NEON_TEXT = [
  { name: 'Neon Rosa', css: 'TARGET { color: #fff !important; text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #f472b6, 0 0 40px #f472b6, 0 0 80px #f472b6 !important; }' },
  { name: 'Neon Grön', css: 'TARGET { color: #fff !important; text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #4ade80, 0 0 40px #4ade80, 0 0 80px #4ade80 !important; }' },
  { name: 'Neon Blå', css: 'TARGET { color: #fff !important; text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #60a5fa, 0 0 40px #60a5fa, 0 0 80px #60a5fa !important; }' },
  { name: 'Neon Guld', css: 'TARGET { color: #fff !important; text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #fbbf24, 0 0 40px #fbbf24, 0 0 80px #fbbf24 !important; }' },
  { name: 'Neon Röd', css: 'TARGET { color: #fff !important; text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #ef4444, 0 0 40px #ef4444, 0 0 80px #ef4444 !important; }' },
  { name: 'Blinkande Neon', css: 'TARGET { color: #fff !important; text-shadow: 0 0 10px #ef4444, 0 0 20px #ef4444, 0 0 40px #ef4444 !important; animation: blinkEffect 0.5s infinite alternate !important; }' },
...
function MittKrypinContent() {
    ...
    async function initData() {
      ...
      if (profileToView.id !== viewer.id) {
        const { data: bData } = await supabase.from('user_blocks').select('*')
          .or(`and(blocker_id.eq.${viewer.id},blocked_id.eq.${profileToView.id}),and(blocker_id.eq.${profileToView.id},blocked_id.eq.${viewer.id})`);
        
        if (bData && bData.length > 0) {
           iBlocked = bData.some(b => b.blocker_id === viewer.id);
           blockedMe = bData.some(b => b.blocker_id === profileToView.id);
      ...
      // Sätt upp realtime listener för Gästboken och Privata Meddelanden
      if(channel) supabase.removeChannel(channel);
      channel = supabase.channel('realtime_krypin_' + profileToView.id)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'guestbook', filter: `receiver_id=eq.${profileToView.id}` }, () => {
          fetchGuestbook(profileToView.id);
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'private_messages', filter: `receiver_id=eq.${viewer.id}` }, () => {
          fetchMessages(viewer.id);
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'private_messages', filter: `sender_id=eq.${viewer.id}` }, () => {
          fetchMessages(viewer.id);
        })
        .subscribe();
      if (profileToView && profileToView.id !== viewer.id) {
      ...
      if (profileToView) {
         if (profileToView.id === viewer.id) {
            ...
            const { data: pubSec } = await supabase.rpc('get_public_user_secrets', { target_user_id: profileToView.id });
            if (pubSec && pubSec.length > 0) {
               setCurrentUser((prev: any) => ({ ...prev, ...pubSec[0] }));
            }
         }
      }
    }
  ...
  useEffect(() => {
    if (chatWith && privateMessages.length > 0 && !selectedMail) {
      const msg = privateMessages.find(m => m.sender_id === chatWith && !m.is_read) 
               || privateMessages.find(m => m.sender_id === chatWith || m.receiver_id === chatWith);
      
      if (msg) {
         setSelectedMail(msg);
         if (!msg.is_read && msg.sender_id !== viewerUser?.id) {
            supabase.from('private_messages').update({ is_read: true }).eq('id', msg.id).then();
            setPrivateMessages(prev => prev.map(m => m.id === msg.id ? {...m, is_read: true} : m));
         }
      }
    }
  }, [chatWith, privateMessages, selectedMail, viewerUser, supabase]);
  ...
  async function fetchGuestbook(userId: string) {
    ...
    if (data) setGuestbookPosts(data);
  }
  ...
  async function fetchFriends(targetId: string, viewerId: string) {
    ...
    if (acceptedData) {
      const friendIds = acceptedData.map(f => f.user_id_1 === targetId ? f.user_id_2 : f.user_id_1);
  ...
  const handleSearchUsers = async (query: string) => {
    ...
    if(data) setComposeSearchResults(data);
  };
  ...
  const fetchRecentVisitors = async () => {
     if(!currentUser) return;
     const { data } = await supabase.from('notifications').select('*, actor:actor_id(username, avatar_url)').eq('receiver_id', currentUser.id).eq('type', 'visit').order('created_at', { ascending: false }).limit(20);
     if(data) {
        const unique = [];
        const seenIds = new Set();
        for (const notif of data) {
           if (!seenIds.has(notif.actor_id)) {
  ...
  const handleToggleBlock = async () => {
    ...
    if (isBlocked) {
      ...
      if(!confirm('Är du säker på att du vill blockera denna användare? Ni kommer inte kunna skicka mejl till varandra eller se varandras inlägg.')) return;
      await supabase.from('user_blocks').insert({ blocker_id: viewerUser.id, blocked_id: currentUser.id });
      setIsBlocked(true);
      setCustomAlert('Användaren har blivit blockerad.');
    }
  };
  ...
  if (!currentUser) return <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg-color)', padding: '2rem' }}>Laddar profil...</div>;
  ...
  return (
    <>
                   ...
                   <div style={{ padding: '2rem' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
                         ...
                         <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', backgroundColor: '#1e293b', padding: '0.25rem', borderRadius: '12px', border: '1px solid #334155', flexWrap: 'wrap' }}>
                            <button onClick={() => setBgColorTarget('.krypin-layout')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.krypin-layout' ? '#a78bfa' : 'transparent', color: bgColorTarget === '.krypin-layout' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>För Bakgrunden</button>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem', marginTop: '1rem' }}>
                         ...
                         <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', backgroundColor: '#1e293b', padding: '0.25rem', borderRadius: '12px', border: '1px solid #334155', flexWrap: 'wrap' }}>
                            <button onClick={() => setTextColorTarget('.krypin-layout')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.krypin-layout' ? '#a78bfa' : 'transparent', color: textColorTarget === '.krypin-layout' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Överallt</button>
                            <button onClick={() => setTextColorTarget('.username-display')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.username-display' ? '#a78bfa' : 'transparent', color: textColorTarget === '.username-display' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Användarnamnet</button>
         ...
         {showReportModal && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
              <div className="card animate-fade-in" style={{ width: '100%', maxWidth: '400px', padding: '2rem' }}>
                 ...
                 <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', fontSize: '0.875rem' }}>Varför vill du anmäla detta inlägg? Beskriv kortfattat (t.ex. spam, trakasserier, olämpligt innehåll).</p>
                 <textarea 
                   value={reportReason}
      ...
      {/* Main Content */}
      <div style={{ flex: 1 }}>
        <div className="card" style={{ minHeight: '500px', padding: '2.5rem' }}>
          ...
          {activeTab === 'Gästbok' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {guestbookPosts.length === 0 && <p style={{ color: 'var(--text-muted)' }}>Gästboken är helt tom än så länge.</p>}
                ...
                {guestbookPosts.map(post => (
                  <div key={post.id} className="card inner-box" style={{ padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', borderLeft: '4px solid var(--theme-krypin)', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', position: 'relative' }}>
                    ...
                    <p style={{ color: 'var(--text-main)', whiteSpace: 'pre-wrap', fontSize: '1.05rem', lineHeight: '1.6' }}>{mask(post.content)}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              {isMyProfile && pendingRequests.length > 0 && (
                <div>
                  <h4 style={{ fontSize: '1.1rem', color: '#f59e0b', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <AlertTriangle size={18} /> Vänförfrågningar ({pendingRequests.length})
                  </h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {activeTab === 'Mejl' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
               <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
                 ...
                 <button onClick={() => setComposeModalOpen(true)} style={{ backgroundColor: 'var(--theme-krypin)', color: 'white', padding: '0.6rem 1.25rem', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>+ Skriv Nytt Mejl</button>
               </div>
               
               {/* COMPOSE SECTION */}
               {composeModalOpen && (
                  <div className="card inner-box" style={{ padding: '1.5rem', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                       <h4 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-main)' }}>Skriv ett nytt mejl</h4>
                       <button onClick={() => { setComposeModalOpen(false); setComposeSelectedUser(null); setComposeSearchQuery(''); setComposeSearchResults([]); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontWeight: 'bold' }}>Stäng (X)</button>
                    </div>

                    {!composeSelectedUser ? (
                       <div>
                         <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 'bold', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Sök mottagare (Användarnamn):</label>
                         <input type="text" value={composeSearchQuery} onChange={e => { setComposeSearchQuery(e.target.value); handleSearchUsers(e.target.value); }} placeholder="Sök mottagare..." style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem' }} />
                         
                         {composeSearchResults.length > 0 && (
                            <div style={{ marginTop: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '8px', backgroundColor: 'white', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                              {composeSearchResults.map(u => (
                                 <div key={u.id} onClick={() => setComposeSelectedUser(u)} style={{ padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }} className="hover-bg-gray">
                                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: 'var(--theme-krypin)', overflow: 'hidden' }}>{u.avatar_url && <img src={u.avatar_url} style={{width:'100%', height:'100%', objectFit:'cover'}}/>}</div>
                                    <strong style={{ color: 'var(--theme-krypin)' }}>@{u.username}</strong>
                                    <span style={{ marginLeft: 'auto', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Välj mottagare 👉</span>
                                 </div>
                              ))}
                            </div>
                         )}
                         {composeSearchQuery.length > 2 && composeSearchResults.length === 0 && (
                            <p style={{ color: '#ef4444', fontSize: '0.875rem', marginTop: '0.5rem' }}>Hittade ingen användare med det namnet.</p>
                         )}
                       </div>
                    ) : (
                       <div className="animate-fade-in">
                         <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', padding: '0.75rem 1rem', backgroundColor: '#e0f2fe', borderRadius: '8px', color: '#0369a1', fontWeight: 'bold' }}>
                            <span>Mottagare vald: <strong style={{ fontSize: '1.1rem' }}>@{composeSelectedUser.username}</strong></span>
                            <button onClick={() => setComposeSelectedUser(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#0369a1', cursor: 'pointer', textDecoration: 'underline', fontSize: '0.875rem' }}>Ändra Mottagare</button>
                         </div>
                         <textarea value={composeContent} onChange={e => setComposeContent(e.target.value)} rows={5} placeholder="Skriv ditt meddelande här..." style={{ width: '100%', padding: '1rem', borderRadius: '8px', border: '1px solid #cbd5e1', resize: 'vertical', marginBottom: '1rem', fontSize: '1rem', fontFamily: 'inherit' }} />
                         <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                           <button onClick={handleSendComposedPM} disabled={!composeContent.trim()} style={{ backgroundColor: 'var(--theme-krypin)', color: 'white', padding: '0.75rem 2rem', borderRadius: '8px', border: 'none', cursor: composeContent.trim() ? 'pointer' : 'default', fontWeight: '900', fontSize: '1.05rem', opacity: composeContent.trim() ? 1 : 0.5, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>Skicka Mejl ✉️</button>
                         </div>
                       </div>
                    )}
                  </div>
               ...
               {privateMessages.length === 0 ? (
                 <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>Din inkorg är tom! Krypinet ekar...</div>
               ) : (
                 <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                   {selectedMail ? (
                      <div className="animate-fade-in">
                        <button onClick={() => setSelectedMail(null)} style={{ background: 'none', border: 'none', color: 'var(--theme-krypin)', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', padding: 0 }}>
                          ⬅️ Tillbaka till Inkorgen
                        </button>
                        <div className="card inner-box" style={{ padding: '1.5rem', border: '1px solid var(--border-color)', borderRadius: '12px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
                             <div>
                               <span className="user-link" onClick={() => window.location.href = `/krypin?u=${(selectedMail.sender_id === viewerUser?.id ? selectedMail.receiver : selectedMail.sender)?.username}`} style={{cursor:'pointer', fontWeight: 'bold', fontSize: '1.1rem'}}>
                                 {selectedMail.sender_id === viewerUser?.id ? `Till: @${selectedMail.receiver?.username}` : `Från: @${selectedMail.sender?.username}`}
                               </span>
                             </div>
                             <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                               <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{new Date(selectedMail.created_at).toLocaleString('sv-SE')}</span>
                               <button onClick={() => handleDeletePM(selectedMail)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: 0, opacity: 0.7 }} title="Radera meddelande"><Trash2 size={18} /></button>
                             </div>
                          </div>
                          <p style={{ margin: '0 0 2rem 0', whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>{mask(selectedMail.content)}</p>
                          
                          <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
                            <textarea value={replyContent} onChange={e => setReplyContent(e.target.value)} rows={4} className="card inner-box" style={{ width: '100%', padding: '1rem', borderRadius: '8px', border: '1px solid #cbd5e1', resize: 'vertical', marginBottom: '1rem', fontFamily: 'inherit', color: 'var(--text-main)' }} placeholder={`Svara @${(selectedMail.sender_id === viewerUser?.id ? selectedMail.receiver : selectedMail.sender)?.username}...`}></textarea>
                            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                              <button onClick={() => { handleSendPM((selectedMail.sender_id === viewerUser?.id ? selectedMail.receiver : selectedMail.sender)?.id, replyContent); setSelectedMail(null); }} disabled={!replyContent.trim()} style={{ padding: '0.75rem 2rem', background: 'var(--theme-krypin)', color: 'white', border: 'none', borderRadius: '8px', cursor: replyContent.trim() ? 'pointer' : 'default', fontWeight: 'bold', opacity: replyContent.trim() ? 1 : 0.5 }}>Skicka Svar ✉️</button>
                            </div>
                          </div>
                        </div>
                      </div>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                       {privateMessages.map(msg => {
                         ...
                         const targetUser = isSentByMe ? msg.receiver : msg.sender;
                         
                         return (
                          <div key={msg.id} onClick={async () => {
                             setSelectedMail(msg);
                             if (!msg.is_read && !isSentByMe) {
                                await supabase.from('private_messages').update({ is_read: true }).eq('id', msg.id);
                                setPrivateMessages(prev => prev.map(m => m.id === msg.id ? {...m, is_read: true} : m));
                             }
                          }} style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)', backgroundColor: (!msg.is_read && !isSentByMe) ? '#fef3c7' : undefined, display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', borderRadius: '8px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }} className="card inner-box hover-lift">
                             <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', overflow: 'hidden' }}>
                               <strong style={{ minWidth: '150px', color: 'var(--text-main)' }}>{isSentByMe ? `Till: @${targetUser?.username}` : `Från: @${targetUser?.username}`}</strong>
                               <span style={{ color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '400px' }}>
                                 {isSentByMe && <span style={{ color: '#94a3b8', marginRight: '0.5rem', fontWeight: 'bold' }}>[Skickat]</span>}
                                 {mask(msg.content)}
                               </span>
                             </div>
                             <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
                               <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{new Date(msg.created_at).toLocaleDateString('sv-SE')}</span>
                               <button onClick={(e) => { e.stopPropagation(); handleDeletePM(msg); }} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: 0, opacity: 0.5 }} title="Radera meddelande"><Trash2 size={16} /></button>
                             </div>
                          </div>
                         );
                       })}
