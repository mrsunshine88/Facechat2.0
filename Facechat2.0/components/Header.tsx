  ...
  useEffect(() => {
    ...
    async function fetchPersonligaNotiser() {
      const { data: notifsData } = await supabase.from('notifications')
         ...
         .neq('type', 'visit') // SILENT LOGGING: Filter out profile visits from the main bell
         .order('created_at', { ascending: false })
         .limit(40);

      const notifs: any[] = [];
      if(notifsData) {
         notifsData.forEach(n => notifs.push({ 
           id: n.id, 
           type: n.type, 
    ...
    // 6. Lyssna på Snake Scores (Global Ticker)
    const snakeChannel = supabase.channel('global-snake-ticker')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'snake_scores' }, (payload) => {
         setSnakeTicker(payload.new.score);
         setIsTickerBlinking(true);
         setTimeout(() => {
            setIsTickerBlinking(false);
            setSnakeTicker(null); // Töm rutan på poängen efter 5 sek
         }, 5000);
      }).subscribe();
    ...
    <>
      <header className="main-header">
        ...
        <div className="header-actions" style={{ gap: '0.5rem' }}>
          {/* SNAKE TICKER */}
          <div onClick={() => router.push('/krypin')} className="hover-lift" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: '80px', gap: '0.3rem', backgroundColor: '#064e3b', padding: '0.2rem 0.5rem', borderRadius: '20px', color: '#34d399', fontWeight: 'bold', fontSize: '0.8rem', border: '1px solid #10b981', boxShadow: '0 0 10px rgba(16,185,129,0.5)', animation: isTickerBlinking ? 'blinkEffect 0.5s infinite alternate' : 'none', whiteSpace: 'nowrap' }}>
            <span>🐍 Score: {snakeTicker !== null ? snakeTicker : '-'}</span>
          </div>
