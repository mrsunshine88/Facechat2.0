...
const target = `                {isComposingNew && (
                  <div className="card inner-box animate-fade-in krypin-compose-card" style={{ padding: '1.5rem', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                       <h4 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-main)' }}>Skriv ett nytt mejl</h4>
                       <button onClick={() => { setIsComposingNew(false); setComposeSearchQuery(''); setComposeSearchResults([]); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontWeight: 'bold' }}>Stäng (X)</button>
                    </div>
                    ...
                    <div>
                       ...
                       <input type="text" value={composeSearchQuery} onChange={e => { setComposeSearchQuery(e.target.value); handleSearchUsers(e.target.value); }} placeholder="Sök mottagare..." style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem', color: '#000' }} />
                       ...
                       {composeSearchResults.length > 0 && (
                          <div style={{ marginTop: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '8px', backgroundColor: 'white', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                            {composeSearchResults.map(u => (
                               <div key={u.id} onClick={() => { setIsComposingNew(false); setSelectedThreadUserId(u.id); setComposeSearchQuery(''); setComposeSearchResults([]); }} style={{ padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }} className="hover-bg-gray">
                                  ...
                                  <span style={{ marginLeft: 'auto', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Starta chatt 👉</span>
                               </div>
                            ))}
                          </div>
                       )}
