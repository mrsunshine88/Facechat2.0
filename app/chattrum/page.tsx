...
import { Send, Users, Hash, Lock, Trash2 } from 'lucide-react';
...
function ChattrumContent() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (data) {
        setRooms(data);
        if (urlRoomName) {
           const targetRoom = data.find(r => r.name === urlRoomName || r.id === urlRoomName);
           if (targetRoom) {
  ...
  const handleJoinRoom = (room: any) => {
    if (activeRoom?.id === room.id) return; // Allt redan klart
  ...
  async function fetchMessages(roomId: string, isFriendsOnly: boolean) {
    ...
    if (currentUser) {
       ...
       if (bData) {
           bData.forEach(b => {
             blockedIds.push(b.blocker_id === currentUser.id ? b.blocked_id : b.blocker_id);
           });
       }
    }
  ...
  const handleSend = async () => {
    ...
    if (participants) {
       const uniqueIds = Array.from(new Set(participants.map(p => p.author_id))).filter(id => id !== currentUser.id);
  ...
  return (
    <div style={{ height: 'calc(100vh - 140px)', display: 'grid', gridTemplateColumns: '250px 1fr 200px', gap: '1.5rem', alignItems: 'stretch' }} className="krypin-layout chat-layout">
      ...
      {/* Vänster: Rumlista */}
      <div className="card krypin-sidebar" style={{ margin: 0, padding: '1rem', display: 'flex', flexDirection: 'column' }}>
        <h2 style={{ fontSize: '1.125rem', marginBottom: '1rem', color: 'var(--text-main)', fontWeight: '600' }}>Rum (Realtime)</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }} className="krypin-sidebar-menu room-list-mobile">
          {rooms.map(room => (
            <button 
              key={room.id}
      ...
      <div className="card chat-main-card" style={{ margin: 0, padding: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-card)', display: 'flex', alignItems: 'center' }}>
        ...
        <div className="chat-messages-container" style={{ flex: 1, minHeight: 0, padding: '1.5rem', overflowY: 'auto', backgroundColor: '#f8fafc', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {!activeRoom && (
            <p style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: '2rem', padding: '2rem' }}>Du måste ansluta till ett rum i listan till vänster för att kunna chatta.</p>
          )}
        ...
        <div className="chat-input-wrapper" style={{ position: 'sticky', bottom: 0, padding: '1rem', paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))', borderTop: '1px solid var(--border-color)', backgroundColor: 'var(--bg-card)', display: 'flex', gap: '0.75rem', zIndex: 10 }}>
          <input 
            type="text" 
            value={inputVal}
      ...
      <style jsx global>{`
        @media (max-width: 768px) {
          ...
          .krypin-sidebar { padding: 0.5rem !important; flex-shrink: 0; }
          .room-list-mobile {
             flex-direction: row !important;
             overflow-x: auto;
             padding-bottom: 0.5rem;
          }
          .room-list-mobile button {
             flex: 0 0 auto;
