  ...
  return (
    <div style={{ height: 'calc(100vh - 140px)', display: 'grid', gridTemplateColumns: '250px 1fr 200px', gap: '1.5rem', alignItems: 'stretch' }} className="krypin-layout chat-layout">
      ...
      <div className={`card chat-main-card ${isSpectator ? 'spectator-mode' : ''}`} style={{ margin: 0, padding: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', border: isSpectator ? '3px solid #ef4444' : '1px solid var(--border-color)', boxShadow: isSpectator ? '0 0 20px rgba(239, 68, 68, 0.2)' : 'none' }}>
        ...
        {/* Mobile Header Toggle (Truly FIXED at top) */}
        <div className="mobile-only-flex" style={{ 
          ...
          backgroundColor: isSpectator ? '#fef2f2' : 'var(--bg-card)', 
          alignItems: 'center', 
          justifyContent: 'space-between',
          boxShadow: '0 2px 10px rgba(0,0,0,0.05)'
        }}>
           <button onClick={() => setMobileDropdownOpen(!mobileDropdownOpen)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: 'var(--theme-chat)', fontWeight: '700', fontSize: '1rem' }}>
           ...
           {activeRoom && activeRoom.is_secret && !isSpectator && (
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                 {canManageMembers && !isAdminRoom && (
                    <button onClick={openInviteModal} style={{ background: 'var(--theme-chat)', color: 'white', border: 'none', padding: '8px', borderRadius: '8px' }} title="Bjud in"><UserPlus size={18} /></button>
                 )}
                 {!isSpectator && !isAdminRoom && (
                    <button onClick={handleLeaveRoom} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '8px', borderRadius: '8px' }} title="Lämna"><LogOut size={18} /></button>
                 )}
              </div>
           )}
        </div>
        ...
        <div className="mobile-only-block" style={{ height: '54px' }}></div>
        ...
        {mobileDropdownOpen && (
          <div className="mobile-only-block" style={{ position: 'fixed', top: '118px', left: 0, right: 0, bottom: 0, zIndex: 900, backgroundColor: 'white', display: 'flex', flexDirection: 'column', gap: '0.8rem', padding: '1.5rem', overflowY: 'auto', animation: 'fadeIn 0.2s ease-out' }}>
