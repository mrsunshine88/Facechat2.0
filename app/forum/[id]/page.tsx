...
export default function ForumThread() {
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    posts.forEach(p => { if(p.author_id) participants.add(p.author_id); });
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1.5rem' }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
             ...
             <h1 style={{ fontSize: '2rem', color: 'var(--text-main)', margin: 0 }}>{mask(thread.title)}</h1>
        ...
        <form onSubmit={handleReply} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
           ...
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
             <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', margin: 0 }}>
               Du postar som: <span style={{ fontWeight: 'bold', color: useAlias ? 'var(--theme-forum)' : 'inherit' }}>{useAlias ? (currentUser?.alias_name || 'Stenansikte') : `@${currentUser?.username}`}</span>
             </p>
      ...
      {showReportModal && (
         <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
           <div className="card animate-fade-in" style={{ width: '100%', maxWidth: '400px', padding: '2rem' }}>
              ...
              <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', fontSize: '0.875rem' }}>Varför vill du anmäla detta inlägg? Beskriv kortfattat.</p>
              <textarea 
                value={reportReason}
