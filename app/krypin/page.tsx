...
import { Mail, Book, Users, Star, Settings, User, FileEdit, Search, Shield, HelpCircle, AlertTriangle, CheckCircle, Eye, Camera, Clock, Trash2, Eraser, ShieldAlert, MessageSquare, Moon, Heart, Zap, Coffee, Ghost, Sun, Gamepad2, Undo, Music, Volume2, VolumeX, ArrowLeft } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function initData() {
      ...
      channel = supabase.channel('realtime_krypin_' + profileToView.id)
        // Globala (utan filter) för raderingar pga Postgres Replica Identity Default
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'guestbook' }, () => fetchGuestbook(profileToView.id))
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'private_messages' }, () => fetchMessages(viewer.id))
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'friendships' }, () => fetchFriends(profileToView.id, viewer.id))
        // Inserts/Updates (med filter för prestanda)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'guestbook', filter: `receiver_id=eq.${profileToView.id}` }, () => fetchGuestbook(profileToView.id))
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'guestbook', filter: `receiver_id=eq.${profileToView.id}` }, () => fetchGuestbook(profileToView.id))
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'private_messages', filter: `receiver_id=eq.${viewer.id}` }, () => fetchMessages(viewer.id))
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'private_messages', filter: `receiver_id=eq.${viewer.id}` }, () => fetchMessages(viewer.id))
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'private_messages', filter: `sender_id=eq.${viewer.id}` }, () => fetchMessages(viewer.id))
                   ...
                   <div style={{ padding: '2rem' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
                         ...
                         <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', backgroundColor: '#1e293b', padding: '0.25rem', borderRadius: '12px', border: '1px solid #334155', flexWrap: 'wrap' }}>
                            <button onClick={() => setBgColorTarget('.krypin-layout')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.krypin-layout' ? '#8b5cf6' : 'transparent', color: bgColorTarget === '.krypin-layout' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>För Bakgrunden</button>
                            <button onClick={() => setBgColorTarget('.card, .inner-box')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.card, .inner-box' ? '#a78bfa' : 'transparent', border: bgColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: bgColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: bgColorTarget === '.card, .inner-box' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>I Rutorna & Inlägg</button>
                            <button onClick={() => setBgColorTarget('.status-badge')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.status-badge' ? '#a78bfa' : 'transparent', border: bgColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: bgColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: bgColorTarget === '.status-badge' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Status-Känslan</button>
                            <button onClick={() => setBgColorTarget('.interest-badge.interest-badge.interest-badge.interest-badge')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.interest-badge.interest-badge.interest-badge.interest-badge' ? '#a78bfa' : 'transparent', border: bgColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: bgColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: bgColorTarget === '.interest-badge.interest-badge.interest-badge.interest-badge' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Intresse-bubblorna</button>
                            <button onClick={() => setBgColorTarget('.krypin-message-bubble:not(.is-me)')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: bgColorTarget === '.krypin-message-bubble:not(.is-me)' ? '#a78bfa' : 'transparent', border: bgColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: bgColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: bgColorTarget === '.krypin-message-bubble:not(.is-me)' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Mottagna Mejl</button>
                          </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem', marginTop: '1rem' }}>
                         ...
                         <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', backgroundColor: '#1e293b', padding: '0.25rem', borderRadius: '12px', border: '1px solid #334155', flexWrap: 'wrap' }}>
                            <button onClick={() => setTextColorTarget('.krypin-layout')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.krypin-layout' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.krypin-layout' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Överallt</button>
                            <button onClick={() => setTextColorTarget('.username-display')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.username-display' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.username-display' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Användarnamnet</button>
                            <button onClick={() => setTextColorTarget('.interest-badge.interest-badge.interest-badge.interest-badge')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.interest-badge.interest-badge.interest-badge.interest-badge' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.interest-badge.interest-badge.interest-badge.interest-badge' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Intressen (Bubblorna)</button>
                            <button onClick={() => setTextColorTarget('.krypin-sidebar')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.krypin-sidebar' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.krypin-sidebar' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Vänstermeny</button>
                            <button onClick={() => setTextColorTarget('.krypin-sidebar + div .card')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.krypin-sidebar + div .card' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.krypin-sidebar + div .card' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Högerspalt</button>
                            <button onClick={() => setTextColorTarget('.krypin-message-bubble:not(.is-me)')} style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', border: 'none', background: textColorTarget === '.krypin-message-bubble:not(.is-me)' ? '#a78bfa' : 'transparent', border: textColorTarget === $2 ? '2px solid white' : '1px solid #334155', boxShadow: textColorTarget === $2 ? '0 0 15px #a78bfa' : 'none', color: textColorTarget === '.krypin-message-bubble:not(.is-me)' ? 'white' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', transition: 'all 0.2s' }}>Mottagna Mejl</button>
                          </div>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: '0.75rem', marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                {guestbookPosts.filter(post => !globalBlockedIds.has(post.sender_id)).map(post => (
                  <div key={post.id} className="card inner-box" style={{ padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', borderLeft: '4px solid var(--theme-krypin)', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', position: 'relative' }}>
                    ...
                    <p style={{ color: 'var(--text-main)', whiteSpace: 'pre-wrap', fontSize: '1.05rem', lineHeight: '1.6' }}>{post.content}</p>
                  </div>
                ))}
              </div>
            </div>
