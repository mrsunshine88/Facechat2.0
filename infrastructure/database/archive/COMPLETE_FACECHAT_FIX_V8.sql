-- =========================================================================
-- COMPLETE FACECHAT FIX (VERSION 8 - THE ULTIMATE CLEANUP)
-- =========================================================================
-- Detta skript fixar:
...
DECLARE
    ...
    deleted_s integer := 0;
BEGIN
    -- A. Cleanup PROFILES (Använd created_at då updated_at saknas)
    WITH d_profiles AS (
        DELETE FROM public.profiles WHERE ctid IN (
            SELECT ctid FROM (
                SELECT ctid, row_number() OVER (PARTITION BY id ORDER BY created_at DESC) as rn FROM public.profiles
            ) t WHERE t.rn > 1
        ) RETURNING 1
